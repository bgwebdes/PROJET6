...
class Mixpanel_Single_Calculator extends Mixpanel {
	...
	public static function register_data() {
		...
		foreach ( CCBFieldsHelper::fields() as $field ) {
			self::add_data( 'Field ' . $field['name'], self::get_calculators_field_count( $field['tag'] ) );
			self::add_data( 'Field Options ' . $field['name'], self::get_calculators_field_option_count( $field['tag'] ) );
		}
		if ( defined( 'CCB_PRO' ) ) {
	...
	public static function get_calculators_field_count( $field_name ) {
		$fields_array = array();
		foreach ( self::$categories_id as $calculator_id ) {
	...
	public static function get_settings( $query_settings ) {
		...
		foreach ( self::$categories_id as $category_id ) {
			$settings         = get_option( 'stm_ccb_form_settings_' . $category_id );
	...
	public static function get_payment_methods( $payment ) {
		...
		foreach ( self::$categories_id as $category_id ) {
			$settings          = get_option( 'stm_ccb_form_settings_' . $category_id )['formFields'];
			$payment_methods[] = $settings['paymentMethods'];
		}
		foreach ( $payment_methods as $payment_method ) {
			return in_array( $payment, $payment_method, true );
		}
	}
	...
	public static function get_horizontal_style_count() {
		$horizontal_count = 0;
		foreach ( self::$categories_id as $category_id ) {
			$settings = get_option( 'stm_ccb_form_settings_' . $category_id )['general'];
